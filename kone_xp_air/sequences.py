...
def write_profile_block(block_bytes, commit_b=0xFF):
    # Page-select flag 0x01 and the commit below (byte 5 = 0xff, checksum over the 76 bytes = block +
    # that 0xff byte) match a live Frida capture of Swarm II writing DPI (capture_swarm_dpi.py). The
    # actual DPI-write bug was the block's bytes 31-32 (protocol.py), not this sequence.
